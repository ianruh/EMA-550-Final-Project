...
# Gotten from here because I couldn't make the one from class work for hyperbolic:
# https://github.com/jorgepiloto/lamberthub/blob/main/src/lamberthub/universal_solvers/izzo.py
def lamberts2(
    r1_vec: np.ndarray,
    r2_vec: np.ndarray,
    dt: float,
...
def lamberts(*,
    ...
    def lamberts_eq(a):
        ...
        if not isHyperbolic:
            ...
            if(dTheta > np.pi):
                beta = -1*beta
            right = np.sqrt((-1*a)**3 / central_body.mu) * (
                    alpha - beta - (np.sinh(alpha) - np.sinh(beta))
                )
        return right - left

    s_mul = -1.0 if isHyperbolic else 1.0
    result = root_scalar(lamberts_eq, bracket=(s_mul*s/2, s_mul*2*s),x0=s_mul*s)
